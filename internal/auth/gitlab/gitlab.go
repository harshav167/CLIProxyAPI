...
func (c *AuthClient) GetPersonalAccessTokenSelf(ctx context.Context, baseURL, token string) (*PersonalAccessTokenSelf, error) {
	...
	var tokenSelf PersonalAccessTokenSelf
	if err := json.Unmarshal(body, &tokenSelf); err != nil {
...
func (c *AuthClient) FetchDirectAccess(ctx context.Context, baseURL, token string) (*DirectAccessResponse, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, NormalizeBaseURL(baseURL)+"/api/v4/code_suggestions/direct_access", nil)
	if err != nil {
	...
	direct.BaseURL = strings.TrimSpace(direct.BaseURL)
	direct.Token = strings.TrimSpace(direct.Token)
...
func (c *AuthClient) DiscoverModels(ctx context.Context, direct *DirectAccessResponse) ([]DiscoveredModel, error) {
	if direct == nil {
		return nil, fmt.Errorf("gitlab discover models: direct access missing")
	}
	baseURL := strings.TrimRight(strings.TrimSpace(direct.BaseURL), "/")
	token := strings.TrimSpace(direct.Token)
	if baseURL == "" || token == "" {
		return nil, fmt.Errorf("gitlab discover models: direct access incomplete")
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, baseURL+"/v1/models", nil)
	if err != nil {
		return nil, fmt.Errorf("gitlab discover models request failed: %w", err)
	}
	...
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("gitlab discover models response read failed: %w", err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("gitlab discover models request failed with status %d: %s", resp.StatusCode, strings.TrimSpace(string(body)))
	}
	...
	if err := json.Unmarshal(body, &payload); err != nil {
		return nil, fmt.Errorf("gitlab discover models response decode failed: %w", err)
	}
