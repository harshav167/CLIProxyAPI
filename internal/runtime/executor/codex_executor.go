...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	// here and clients like Cursor rely on it to extend upstream cache TTL.
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, userPayload []byte, rawJSON []byte) (*http.Request, []byte, codexIdentityConfuseState, error) {
	...
	if sourceFormatEqual(from, sdktranslator.FormatClaude) {
		...
		if pck := strings.TrimSpace(gjson.GetBytes(req.Payload, "prompt_cache_key").String()); pck != "" {
			// Preserve a client-supplied prompt_cache_key (our cache-burn work)
			// in preference to deriving one from the API key.
			cache.ID = pck
...
func applyCursorGPTUpgradeIfEnabled(ctx context.Context, cfg *config.Config, model string, body []byte) []byte {
	if cfg == nil || !cfg.GPTUpgrade || !IsCursorClient(ctx) || !isGPTModelName(model) {
		return body
	}
	return helps.ApplyCursorGPTSystemPromptUpgradeToPayload(body)
}
