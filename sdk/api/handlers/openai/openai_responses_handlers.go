...
// the "/passthrough" prefix so OpenAI SDK clients can point baseUrl at
// "…/v1/passthrough" and still resolve "/responses" natively). It behaves
// exactly like Responses but flags the request so CodexAutoExecutor bypasses
// the WS bridge and uses the legacy HTTP executor end-to-end. Intended for
// benchmarks and diagnostics — not part of Droid's production path.
func (h *OpenAIResponsesAPIHandler) ResponsesPassthrough(c *gin.Context) {
	c.Set("codex-force-passthrough", true)
	h.Responses(c)
}
