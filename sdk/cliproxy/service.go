...
func (s *Service) registerExecutorForAuth(a *coreauth.Auth, forceReplace bool) {
	...
	case "xai", "xai-oauth", "grok", "grok-oauth":
		// Adopt upstream's XAIAutoExecutor (HTTP + websockets auto-routing); it
		// wraps NewXAIExecutor internally, so all our Grok fixes still apply.
		// Keep our fork's broader provider aliases (xai-oauth/grok/grok-oauth).
...
// applyObservabilityConfig (fork) restarts the OTel pipeline on config reload so
// SigNoz keeps receiving traces/metrics/logs after a hot reload.
